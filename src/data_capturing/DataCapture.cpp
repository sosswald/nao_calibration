...
void DataCapture::moveArmDown() {
	/*trajectory_msgs::JointTrajectoryPoint point;
	point.time_from_start.sec = 0;
	point.time_from_start.nsec = 600 * 1000 * 1000;
	if(jointNames[0].find("L")==0){
		point.positions.push_back(1.40817);
		point.positions.push_back(0.177902);
		point.positions.push_back(-0.8238);
		point.positions.push_back(-1.04461);
    ...
    goal.trajectory.points[step].positions.resize(10);
    goal.trajectory.points[step].positions[0] = -0.42641;
    goal.trajectory.points[step].positions[1] = -1.18429;
    goal.trajectory.points[step].positions[2] = 1.47567;
    goal.trajectory.points[step].positions[3] = 0.154892;
    goal.trajectory.points[step].positions[4] = 0.0720561;
    goal.trajectory.points[step].positions[5] = 0.408086;
    goal.trajectory.points[step].positions[6] = 1.20722;
    goal.trajectory.points[step].positions[7] = 1.48035;
    goal.trajectory.points[step].positions[8] = -0.181054;
    goal.trajectory.points[step].positions[9] = 0.07359;
    goal.trajectory.points[step].velocities.resize(10);
    for (size_t j = 0; j < goal.trajectory.joint_names.size(); ++j)
    {
        goal.trajectory.points[step].velocities[j] = 0.0;
    }

    goal.trajectory.points[step].time_from_start = ros::Duration(3);

    trajectoryClient.sendGoal(goal);
    trajectoryClient.waitForResult();

    ROS_INFO("Stiffness: %f", 0.0);
    setStiffness(goal.trajectory.joint_names, 0.0);
}
...
vector<double> DataCapture::generateMoveDownPosition(
		const vector<string>& joints) {
	// TODO: parameterize!
	double low = -0.10, high = 0.10;
	vector<double> positions;
	const vector<string>& jointNames = this->getJointNames();
        positions.push_back(1.40817);
	positions.push_back(0.177902);
	positions.push_back(-0.8238);
	positions.push_back(-1.04461);
	positions.push_back(0.128814);
	positions.push_back(0.0104001);
	return positions;
}
